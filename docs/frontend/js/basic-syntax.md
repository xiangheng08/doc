...
JavaScript 使用大括号，将多个相关的语句组合在一起，称为“区块”（block）或者“代码块”。
...
`for...in` 用于遍历对象的可枚举属性，包括对象自身的属性和原型链上的属性。
...
for (const item of arr) {
	console.log(item); // item 就是数组的每个值
}
```

> 注意，for...of 循环不能遍历对象，因为对象不是可迭代对象。

for...of 除了遍历数组，还可以遍历：`String`、`Map`、`Set`、`TypedArray` 或者自定义可迭代对象。
...
const customIterable = {
	...
	[Symbol.iterator]: function () {
		...
		return {
			next: () => {
				return index < this.values.length ? { value: this.values[index++], done: false } : { done: true };
			},
		};
	},
};
...
[参考: JavaScript 的基本语法 - JavaScript 教程 - 网道](https://wangdoc.com/javascript/basic/grammar)
