...
通配符选择器可以匹配任何标签，我们常用于统一页面样式、清除默认的样式等
...
兄弟选择器选取所有指定元素之后的相邻兄弟元素。定义的时候用 ~ 隔开
...
相邻兄弟选择器**可选择紧接在另一元素后的元素，且二者有相同父元素**。如果需要选择紧接在另一个元素后的元素，而且二者有相同的父元素，定义的时候用 + 隔开。
...
当多个元素共享样式时，这时候就可以使用并集选择器。定义的时候每个选择器用逗号(,)隔开。
...
-   **特殊性**：每一个选择器都会有一个四位数字组成特殊性的值
    ...
    浏览器会依次比较的每一位的值，假如第一位就比较出来了大小，那么后面的就忽略，星号选择器（\*）的特殊性为 0
    ...
    * {
    ...
    > 其实选择器的特殊性，在 vscode 中把鼠标放到选择器上就显示特殊性（只有后三位）
