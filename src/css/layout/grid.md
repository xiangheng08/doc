...
CSS Grid（网格布局）是 CSS 中最强大的二维布局系统，能够同时控制行和列，从而实现复杂而灵活的页面结构。

<GridDemo />

Grid 是一种基于网格的布局模型，适用于构建复杂的二维布局（行 + 列）。与 Flex 基于轴线的一维布局不同，Grid 能同时控制水平和垂直方向，是构建现代网页布局的首选方案。
...
这里，最外层的 `<div>` 元素就是容器，内层的三个 `<div>` 元素就是项目。
...
有时，单元格的大小是固定的，但是容器的大小不确定。如果希望每一行（或每一列）容纳尽可能多的单元格，这时可以使用 `auto-fill` 或者 `auto-fit` 关键字表示自动填充。
...
- `auto-fill`：尽可能填满轨道，**即使没有内容，空轨道也会保留**（占位）。
- `auto-fit`：尽可能填满轨道，**如果没有内容，空轨道会折叠消失**，现有内容会拉伸填满空间。

> 注意：只有当网格项目（items）不足以填满容器时，两者的区别才会显现。如果项目足够多，填满了所有行，它们的表现是完全一样的。
...
.container {
  ...
  grid-gap: 20px; /* grid-gap 是 gap 的别名，等同于 gap */
  grid-gap: 20px 30px;
	grid-row-gap: 20px;
...
上面的 demo 中，1 号项目后面的位置是空的，这是因为 3 号项目默认跟着 2 号项目，所以会排在 2 号项目后面。
...
-   `start` - 对齐容器的起始边框。
    <img src="../images/grid/justify-align-content-start.png" alt="alt" style="width: 40%"/>
-   `end` - 对齐容器的结束边框。
...
-   `stretch` - 项目大小没有指定时，拉伸占据整个网格容器。
    <img src="../images/grid/justify-align-content-stretch.png" alt="alt" style="width: 40%"/>
...
-   `space-evenly` - 项目与项目的间隔相等，项目与容器边框之间也是同样长度的间隔。
    <img src="../images/grid/justify-align-content-space-evenly.png" alt="alt" style="width: 40%"/>
